...
- 各候補について出力前に「なぜそうなったのか」と「どのように実現・変化・保存されたのか」を一つずつ問い、explanation内で両方へ直接答える
...
【内部評価】
候補を広く調査してから、「意外性」「場所との強い結びつき」「現地で確認できる度合い」「地元資料ならではのニッチさ」「なぜ・どのようにの説明の完成度」「深掘りの面白さ」「信頼性」「話したくなるか」「行きたくなるか」を各0〜5点で評価する。
