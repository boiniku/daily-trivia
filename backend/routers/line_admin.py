...
def candidate_editor(candidate_id: int, token: str):
    ...
    try:
        ...
        return HTMLResponse(_editor_html(candidate, token))
    finally:
        db.close()
...
def _editor_html(candidate: TriviaCandidate, token: str) -> str:
    options = "".join(
...
async function submitCandidate(publish){{
 ...
 const response=await fetch(window.location.pathname,{{method:"PUT",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{
 ...
 source:document.getElementById("source").value,image_url:document.getElementById("image_url").value,publish}})}});
