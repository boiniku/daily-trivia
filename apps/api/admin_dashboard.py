...
def _map_trivia_to_admin_spot(trivia: MapTrivia) -> dict:
    return {
        ...
        "unlockRadiusMeters": int(trivia.map_radius or 1200),
        "isUnlocked": False,
        "unlockedAt": None,
...
def render_trivia_map_admin():
    ...
    m_radius = st.number_input(
        ...
        min_value=10,
        max_value=5000,
        value=int(spot.get("unlockRadiusMeters") or 1200),
        step=10,
    ...
    with st.form("register_form", clear_on_submit=True):
        ...
        with col1:
            ...
            if add_to_trivia_map:
                ...
                map_radius = st.number_input("解放半径（メートル）", min_value=10, max_value=5000, value=1200, step=10)
                map_hint = ""
            else:
                map_address = ""
                map_prefecture = ""
                ...
                with st.expander(f"WAITING: {item.get('title', '無題')}", expanded=True):
                    with st.form(f"approve_form_{i}"):
                        ...
                        if a_add_to_map:
                            ...
                            a_map_radius = st.number_input("解放半径（メートル）", min_value=10, max_value=5000, value=int(item.get("map_radius") or 1200), step=10, key=f"ai_map_radius_{i}")
                            a_map_hint = ""
                        else:
                            a_map_prefecture = ""
                            a_map_latitude = 0.0
        ...
        if trivia:
            ...
            with st.container():
                ...
                with st.expander("この雑学を雑学MAPに登録"):
                    ...
                    e_map_radius = st.number_input("解放半径（メートル）", min_value=10, max_value=5000, value=1200, step=10, key=f"edit_map_radius_{trivia.id}")
                    e_map_hint = ""
