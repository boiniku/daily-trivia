...
def build_collection_prompt(
    ...
    if map_mode:
        ...
        search_depth_rules = """- 起点となる事実を見つけたら「なぜそうなったのか」「例外はあるか」「一見矛盾する事例はないか」「名称・商標・制度・歴史とどう繋がるか」のうち、最も面白くなる問いを1つ立て、その答えを追加検索する
- 例: 「宅急便は商標」で止めず、「それなら『魔女の宅急便』でなぜ使えるのか」を検索する。検索ページで理由まで確認できた場合だけ候補にする
- 1段目の事実だけよりも、2段目の理由・例外・意外な繋がりまで確認できた候補を優先する"""
        selection_rules = """- 見つけた候補をすぐ採用せず、「予想とのギャップ」「次の疑問」「深掘りした答えの面白さ」「身近さ・話しやすさ」「出典の信頼性」を各0〜5点で内部評価する
- 合計18点未満の候補は出力しない
- 「深掘りした答え」または「予想を超える規模を実感できる面白さ」が4点未満の候補は出力しない"""
        number_rules = """- 「最大」「最速」「最古」「非常に多い」という記録だけでは採用しない
- 数字を出す場合は、身近な物・人間・建物・時間・距離などとの正確な比較、予想との大きな差、またはその規模になる面白い理由を最低1つ入れる"""
        source_rule = "- sourceには、最終的な深掘り内容を直接説明している個別ページのhttpまたはhttps URLを入れる"
        strict_rejection_examples = """- 世界最大・最速・最古という記録だけで、身近な比較や面白い理由がないもの
...
- 有名な定番雑学を、深掘りや新しい繋がりなしで言い直したもの"""
        ...
        explanation_rule = (
            "- explanationは60〜120文字程度、最大2文で、contentの繰り返しではなく、個別記事で確認した"
            "理由、仕組み、背景、条件、例外のうち書けるものを平易に補足する"
        )
        content_example = "中学生が一読で分かる45〜75文字程度の本文"
        explanation_example = "記事で確認できる補足を平易に説明する60〜120文字、最大2文の解説"
        search_depth_rules = """- 中心事実と説明を個別記事で確認する。記事内に理由、由来、仕組み、例外があればexplanationへ活用する
- 追加の深掘り検索は、候補の面白さや正確さを高める場合だけ行えばよく、必須ではない
- 理由や二段目の意外性がなくても、一文で明確に伝わる具体的な雑学なら候補にしてよい"""
        selection_rules = """- 個別記事で確認できる具体的な事実で、専門知識なしに意味が分かれば基本的に採用してよい
- 面白さの評価は候補を並べ替えるために使い、点数による足切りはしない
- 定番の雑学でも、既存データと重複せず、記事で確認できるなら採用してよい"""
        number_rules = """- 最大・最速・最古などの記録も、数字と対象が記事で明確に確認できれば採用してよい
- 比較や理由は記事にある場合だけ補足し、出典にない比較を作らない"""
        source_rule = "- sourceには、中心事実を直接説明している許可サイト内の個別記事URLを入れる"
        strict_rejection_examples = "- 内容が広すぎて、具体的な事実を一つも言えないもの"
    return f"""
