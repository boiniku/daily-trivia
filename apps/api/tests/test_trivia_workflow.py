...
class LineSecurityTests(unittest.TestCase):
    ...
    def test_collection_prompt_forbids_copying(self):
        ...
        self.assertIn("中心事実を直接説明している許可サイト内の個別記事URL", prompt)
        ...
        self.assertIn("追加の深掘り検索", prompt)
        self.assertIn("必須ではない", prompt)
        self.assertIn("一文で明確に伝わる具体的な雑学なら候補にしてよい", prompt)
        ...
        self.assertIn("点数による足切りはしない", prompt)
        self.assertIn("定番の雑学でも", prompt)
        self.assertIn("身近な比較で規模を実感できる場合は積極的に採用", prompt)
        self.assertIn("記録も、数字と対象が記事で明確に確認できれば採用", prompt)
        ...
        self.assertIn("60〜120文字程度、最大2文", prompt)
    ...
    def test_map_collection_prompt_requires_place_fields(self):
        ...
        self.assertIn("深掘り内容を直接説明している個別ページ", prompt)
    ...
    def test_normal_collection_prompt_keeps_compact_length(self):
        ...
        self.assertIn("explanationは60〜120文字程度", prompt)
